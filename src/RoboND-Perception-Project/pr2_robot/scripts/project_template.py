...
def pr2_mover(object_list):
    ...
    #pcl_objects_pub.publish(object_list)
    #pcl_table_pub.publish(object_list)    
    dict_list = []
    ...
    for object in object_list:
        ...
        tmp = [np.asscalar(x) for x in np.mean(points_arr, axis=0)[:3]]
        
        
        
        PICK_POSE = Pose()
        PICK_POSE.position.x = tmp[0]
        PICK_POSE.position.y = tmp[1]
        PICK_POSE.position.z = tmp[2]
